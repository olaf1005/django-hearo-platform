...
#
# Based on wav2png.py by Bram de Jong <bram.dejong at domain.com where domain
# in gmail>
...
class AudioProcessor(object):
    ...
    def read(self, start, size, resize_if_less=False, channel=-1):
        ...
        less than size samples are read, resize the array to size and fill with
        zeros """
    ...
    def spectral_centroid(self, seek_point, channel, spec_range=120.0):
        ...
        samples = self.read(
            seek_point - self.fft_size / 2, self.fft_size, True, channel
        )

        samples *= self.window
        fft = numpy.fft.fft(samples)
        spectrum = numpy.abs(fft[: fft.shape[0] / 2 + 1]) / float(
            self.fft_size
        ...
        db_spectrum = (
            (20 * (numpy.log10(spectrum + 1e-30))).clip(-spec_range, 0.0) + spec_range
        ) / spec_range
    ...
    def save(self, filename):
        ...
        for x in range(self.image_width):
            self.pix[x, self.image_height / 2] = tuple(
                [p + a for p in self.pix[x, self.image_height / 2]]
            )

        alpha = self.image.split()[0]
        self.image = self.image.convert("RGB").convert(
...
def create_png(
    ...
    for channel in range(channels):
        ...
        if channels == 1:
            ...
            waveform.save(os.path.join(path_split[0], str(channel) + path_split[1]))
    ...
    if channels > 1:
        ...
        # Delete the segments
        for channel in range(channels):
            cur = Image.open(os.path.join(path_split[0], str(channel) + path_split[1]))
            combined.paste(cur, (0, channel * (image_height / channels)))
            os.remove(os.path.join(path_split[0], str(channel) + path_split[1]))
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="f_max",
        type="int",
        help="Maximum freq to draw, in Hz (default %default)",
    )
    parser.add_option(
        "-i",
        "--fmin",
        ...
        dest="f_min",
        type="int",
        help="Minimum freq to draw, in Hz (default %default)",
    )
    parser.add_option(
        "-v",
        "--version",
    ...
    if not options.version:
        ...
        if len(args) > 1 and (
            options.output_filename_w is not None
            or options.output_filename_s is not None
        ):
            parser.error(
                "when processing multiple files you can't "
