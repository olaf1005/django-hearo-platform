#!/usr/bin/env python
# https://raw.githubusercontent.com/Concert/Concert/master/concertapp/audio/audioHelpers.py
...
def toNormalizedWav(inputFilePath, outputFilePath):
    ...
    normalizedPCM = audiotools.PCMConverter(
        origPCM, 44100, 2, channel_mask, bits_per_sample
    )

    # Output normalized audio to wav (Can raise error)
    wav = audiotools.WaveAudio.from_pcm(outputFilePath, normalizedPCM)
...
def toOgg(inputFilePath, outputFilePath):
    ogg = audiotools.VorbisAudio.from_pcm(
        outputFilePath, audiotools.open(inputFilePath).to_pcm()
    )
    os.chmod(outputFilePath, NEW_FILE_PERMISSIONS)
...
def toMp3(inputFilePath, outputFilePath):
    mp3 = audiotools.MP3Audio.from_pcm(
        outputFilePath, audiotools.open(inputFilePath).to_pcm()
    )
    os.chmod(outputFilePath, NEW_FILE_PERMISSIONS)
